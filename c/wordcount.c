...
hash_rec* hash_table;
const float rehash_constant = 0.8f;
const float expand_constant = 1.5f;
...
INSERT_RESULT compare_hash(hash_rec* where, const char* str, size_t len)
{
	if (where->str == NULL)
	{
...
void hash_insert(const char* str, size_t len, hash_rec* hash_table, size_t hash_table_size)
{
	...
	{
		switch (compare_hash(where, str, len))
		{
		case INCREMENTED:
		case INSERTED:
			return;
...
int main(int argc, char* argv[])
{
	...
	while (scanf(input_format, word) == 1)
	{
		len = strlen(word);
		*((size_t*)(word + len)) = 0;
		hash_insert(word, len, hash_table, hash_table_size);
